...
private:
    ...
    QAction* specialStatusAction;           //触发特殊状态选项
    QAction* setAction;                     //设置选项
    QAction* closeAction;                   //关闭选项
    QAction* performanceWindowAction;       //性能窗口选项
    QAction* customModelAction;             //自定义模型选项
    std::shared_ptr<SetWindow> setWindowPtr;//设置窗口对象指针
    bool isOpenSetWindow;                   //是否打开设置窗口
    bool isRightMenu;                       //是否右键了菜单
    PerformanceWindow performanceWindow;    //设置窗口对象指针
